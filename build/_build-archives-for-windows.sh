...
if $( [ ${BUILD_COUNT} -ne ${#HOST_VS_PRODUCT_NAMES[@]} ] || [ ${BUILD_COUNT} -ne ${#TARGET_ARCH_TYPES[@]} ] || [ ${BUILD_COUNT} -ne ${#TARGET_ARCH_NAMES[@]} ] ); then
...
for (( i = 0; i < ${BUILD_COUNT}; ++i )); do
    ...
    ARCHIVE_NAME="clang-server-${SERVER_VERSION}-${TARGET_ARCH_NAME}-vs${HOST_VS_PRODUCT_NAME}"
    INSTALL_PREFIX=$( cygpath -am "${WORK_DIR}/${ARCHIVE_NAME}" )

    cmd /c "build.bat ${HOST_VS_PRODUCT_NAME} ${TARGET_LLVM_VERSION} ${TARGET_ARCH_TYPE} Release ${INSTALL_PREFIX}"
